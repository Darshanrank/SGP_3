...
const Swaps = () => {
    ...
    const [activeTab, setActiveTab] = useState('received'); // 'received', 'sent', 'active'
    ...
    const fetchData = async () => {
        try {
            const [sent, received, classes] = await Promise.all([
                getMyRequests('sent'),
                getMyRequests('received'),
                getMyClasses()
            ]);
            setSentRequests(sent?.data || sent || []);
            setReceivedRequests(received?.data || received || []);
            setActiveClasses(classes?.data || classes || []);
        } catch (error) {
                    ...
                    <div className="space-y-4">
                        ...
                        {sentRequests.map(req => (
                            <div key={req.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                                ...
                                <p className="text-sm text-gray-600">Skill: {req.learnSkill?.skill?.name || 'Skill not found'}</p>
                                <div className={`text-xs mt-2 font-semibold ${req.status === 'PENDING' ? 'text-yellow-600' : req.status === 'ACCEPTED' ? 'text-green-600' : 'text-red-600'}`}>
                                    Status: {req.status}
                                </div>
                            </div>
                        ))}
